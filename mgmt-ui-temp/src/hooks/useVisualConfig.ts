...
import type {
  ...
  PayloadRule,
  VisualHealthCheckProviderPolicy,
  VisualConfigValues,
...
function parseHealthCheckProviderPolicies(raw: unknown): VisualHealthCheckProviderPolicy[] {
  const record = asRecord(raw);
  if (!record) return [];

  return Object.entries(record).map(([provider, policy], index) => {
    const policyRecord = asRecord(policy);
    const unauthorizedRaw = policyRecord?.['unauthorized-action'] ?? policyRecord?.unauthorizedAction;
    const zeroQuotaRaw = policyRecord?.['zero-quota-action'] ?? policyRecord?.zeroQuotaAction;
    return {
      id: `health-policy-${index}`,
      provider,
      unauthorizedAction:
        unauthorizedRaw === 'delete' || unauthorizedRaw === 'disable' || unauthorizedRaw === 'ignore'
          ? unauthorizedRaw
          : 'default',
      zeroQuotaAction: zeroQuotaRaw === 'disable' || zeroQuotaRaw === 'ignore' ? zeroQuotaRaw : 'default',
    };
  });
}
...
export function getVisualConfigValidationErrors(
  ...
  return {
    ...
    'healthCheck.intervalSeconds': getNonNegativeIntegerError(values.healthCheck.intervalSeconds),
    'healthCheck.parallelism': getNonNegativeIntegerError(values.healthCheck.parallelism),
    'healthCheck.email.smtpPort': getNonNegativeIntegerError(values.healthCheck.notifications.emailSmtpPort),
...
export function useVisualConfig() {
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const healthCheck = asRecord(parsed['health-check']);
      const healthNotifications = asRecord(healthCheck?.notifications);
      const barkNotifications = asRecord(healthNotifications?.bark);
      const emailNotifications = asRecord(healthNotifications?.email);
      ...
      const newValues: VisualConfigValues = {
        ...
        healthCheck: {
          ...
          intervalSeconds: String(healthCheck?.['interval-seconds'] ?? ''),
          parallelism: String(healthCheck?.parallelism ?? ''),
          unauthorizedAction:
            healthCheck?.['unauthorized-action'] === 'delete' ||
            healthCheck?.['unauthorized-action'] === 'ignore'
              ? (healthCheck['unauthorized-action'] as 'delete' | 'ignore')
              : 'disable',
          zeroQuotaAction:
            healthCheck?.['zero-quota-action'] === 'ignore' ? 'ignore' : 'disable',
          ...
          notifications: {
            ...
            emailSmtpHost:
              typeof emailNotifications?.['smtp-host'] === 'string' ? emailNotifications['smtp-host'] : '',
            emailSmtpPort: String(emailNotifications?.['smtp-port'] ?? ''),
            emailUsername:
              typeof emailNotifications?.username === 'string' ? emailNotifications.username : '',
            emailPassword:
              typeof emailNotifications?.password === 'string' ? emailNotifications.password : '',
            emailFrom: typeof emailNotifications?.from === 'string' ? emailNotifications.from : '',
            emailTo: Array.isArray(emailNotifications?.to)
              ? emailNotifications.to.map((item) => String(item).trim()).filter(Boolean)
              : [],
            emailSubjectPrefix:
              typeof emailNotifications?.['subject-prefix'] === 'string'
                ? emailNotifications['subject-prefix']
                : '',
          },
        },
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        const healthCheckDefined =
          docHas(doc, ['health-check']) ||
          values.healthCheck.enabled ||
          values.healthCheck.intervalSeconds.trim() ||
          values.healthCheck.parallelism.trim() ||
          ...
          values.healthCheck.notifications.emailUsername.trim() ||
          values.healthCheck.notifications.emailPassword.trim() ||
          values.healthCheck.notifications.emailFrom.trim() ||
          values.healthCheck.notifications.emailTo.length > 0 ||
          values.healthCheck.notifications.emailSubjectPrefix.trim();
        if (healthCheckDefined) {
          ensureMapInDoc(doc, ['health-check']);
          setBooleanInDoc(doc, ['health-check', 'enabled'], values.healthCheck.enabled);
          setIntFromStringInDoc(
            doc,
            ['health-check', 'interval-seconds'],
            values.healthCheck.intervalSeconds
          );
          setIntFromStringInDoc(doc, ['health-check', 'parallelism'], values.healthCheck.parallelism);
          doc.setIn(['health-check', 'unauthorized-action'], values.healthCheck.unauthorizedAction);
          doc.setIn(['health-check', 'zero-quota-action'], values.healthCheck.zeroQuotaAction);

          const providerPolicies = values.healthCheck.providerPolicies
            .map((policy) => ({
              provider: policy.provider.trim(),
              unauthorizedAction:
                policy.unauthorizedAction === 'default' ? undefined : policy.unauthorizedAction,
              zeroQuotaAction:
                policy.zeroQuotaAction === 'default' ? undefined : policy.zeroQuotaAction,
            }))
            .filter((policy) => policy.provider);
          if (providerPolicies.length > 0) {
            doc.setIn(
              ['health-check', 'provider-policies'],
              Object.fromEntries(
                providerPolicies.map((policy) => [
                  policy.provider,
                  {
                    ...(policy.unauthorizedAction
                      ? { 'unauthorized-action': policy.unauthorizedAction }
                      : {}),
                    ...(policy.zeroQuotaAction
          ...
          } else if (docHas(doc, ['health-check', 'provider-policies'])) {
            doc.deleteIn(['health-check', 'provider-policies']);
          }

          const notifications = values.healthCheck.notifications;
          const hasNotifications =
            ...
            notifications.emailUsername.trim() ||
            notifications.emailPassword.trim() ||
            notifications.emailFrom.trim() ||
            notifications.emailTo.length > 0 ||
            notifications.emailSubjectPrefix.trim() ||
            docHas(doc, ['health-check', 'notifications']);
          if (hasNotifications) {
            ensureMapInDoc(doc, ['health-check', 'notifications']);
            ensureMapInDoc(doc, ['health-check', 'notifications', 'bark']);
            setBooleanInDoc(
              doc,
              ['health-check', 'notifications', 'bark', 'enabled'],
              notifications.barkEnabled
            );
            setStringInDoc(
              doc,
            ...
            setBooleanInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'enabled'],
              notifications.emailEnabled
            );
            setStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'smtp-host'],
              notifications.emailSmtpHost
            );
            setIntFromStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'smtp-port'],
              notifications.emailSmtpPort
            );
            setStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'username'],
              notifications.emailUsername
            );
            setStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'password'],
              notifications.emailPassword
            );
            setStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'from'],
              notifications.emailFrom
            );
            if (notifications.emailTo.length > 0) {
              doc.setIn(
                ['health-check', 'notifications', 'email', 'to'],
                notifications.emailTo.filter(Boolean)
              );
            } else if (docHas(doc, ['health-check', 'notifications', 'email', 'to'])) {
              doc.deleteIn(['health-check', 'notifications', 'email', 'to']);
            }
            setStringInDoc(
              doc,
              ['health-check', 'notifications', 'email', 'subject-prefix'],
              notifications.emailSubjectPrefix
            );
            deleteIfMapEmpty(doc, ['health-check', 'notifications', 'email']);
            deleteIfMapEmpty(doc, ['health-check', 'notifications']);
          } else if (docHas(doc, ['health-check', 'notifications'])) {
            doc.deleteIn(['health-check', 'notifications']);
          }

          deleteIfMapEmpty(doc, ['health-check']);
        }
  ...
  const setVisualValues = useCallback((newValues: Partial<VisualConfigValues>) => {
    setVisualValuesState((prev) => {
      ...
      if (newValues.healthCheck) {
        next.healthCheck = {
          ...prev.healthCheck,
          ...newValues.healthCheck,
          notifications: {
            ...prev.healthCheck.notifications,
            ...newValues.healthCheck.notifications,
          },
        };
      }
