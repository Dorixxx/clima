...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const healthCheck = raw['health-check'] ?? raw.healthCheck;
  if (isRecord(healthCheck)) {
    const intervalRaw = healthCheck['interval-seconds'] ?? healthCheck.intervalSeconds;
    const parallelismRaw = healthCheck.parallelism;
    const unauthorizedRaw = healthCheck['unauthorized-action'] ?? healthCheck.unauthorizedAction;
    const zeroQuotaRaw = healthCheck['zero-quota-action'] ?? healthCheck.zeroQuotaAction;
    const providerPoliciesRaw = healthCheck['provider-policies'] ?? healthCheck.providerPolicies;
    const notificationsRaw = healthCheck.notifications;
    const normalizeUnauthorizedAction = (
      value: unknown
    ): 'delete' | 'disable' | 'ignore' | undefined => {
      if (typeof value !== 'string') {
        return undefined;
      }
      const normalized = value.trim();
      if (normalized === 'delete' || normalized === 'disable' || normalized === 'ignore') {
        return normalized;
      }
      return undefined;
    };
    const normalizeZeroQuotaAction = (value: unknown): 'disable' | 'ignore' | undefined => {
      if (typeof value !== 'string') {
        return undefined;
      }
      const normalized = value.trim();
      if (normalized === 'disable' || normalized === 'ignore') {
        return normalized;
      }
      return undefined;
    };
    const providerPolicies =
      isRecord(providerPoliciesRaw)
        ? Object.fromEntries(
            Object.entries(providerPoliciesRaw).map(([provider, value]) => [
              provider,
              {
                unauthorizedAction: isRecord(value)
                  ? normalizeUnauthorizedAction(value['unauthorized-action'] ?? value.unauthorizedAction)
                  : undefined,
                zeroQuotaAction: isRecord(value)
                  ? normalizeZeroQuotaAction(value['zero-quota-action'] ?? value.zeroQuotaAction)
                  : undefined
              }
            ])
          )
        : undefined;
    const notifications = isRecord(notificationsRaw)
      ? {
            ...
            ? {
                ...
                serverUrl:
                  typeof notificationsRaw.bark['server-url'] === 'string'
                    ...
                    : typeof notificationsRaw.bark.serverUrl === 'string'
                      ? notificationsRaw.bark.serverUrl
                      : undefined,
                deviceKey:
                  typeof notificationsRaw.bark['device-key'] === 'string'
                    ? notificationsRaw.bark['device-key']
                    : typeof notificationsRaw.bark.deviceKey === 'string'
                      ? notificationsRaw.bark.deviceKey
                      : undefined,
                group:
                  typeof notificationsRaw.bark.group === 'string' ? notificationsRaw.bark.group : undefined
              }
            : undefined,
          email: isRecord(notificationsRaw.email)
            ? {
                enabled: normalizeBoolean(notificationsRaw.email.enabled),
                smtpHost:
                  typeof notificationsRaw.email['smtp-host'] === 'string'
                    ? notificationsRaw.email['smtp-host']
                    : typeof notificationsRaw.email.smtpHost === 'string'
                      ? notificationsRaw.email.smtpHost
                      : undefined,
                smtpPort:
                  typeof notificationsRaw.email['smtp-port'] === 'number'
                    ? notificationsRaw.email['smtp-port']
                    : typeof notificationsRaw.email.smtpPort === 'number'
                      ? notificationsRaw.email.smtpPort
                      : undefined,
                username:
                  typeof notificationsRaw.email.username === 'string' ? notificationsRaw.email.username : undefined,
                password:
                  typeof notificationsRaw.email.password === 'string' ? notificationsRaw.email.password : undefined,
                from: typeof notificationsRaw.email.from === 'string' ? notificationsRaw.email.from : undefined,
                to: Array.isArray(notificationsRaw.email.to)
                  ? notificationsRaw.email.to.map((item) => String(item).trim()).filter(Boolean)
                  : undefined,
                subjectPrefix:
                  typeof notificationsRaw.email['subject-prefix'] === 'string'
                    ? notificationsRaw.email['subject-prefix']
                    : typeof notificationsRaw.email.subjectPrefix === 'string'
                      ? notificationsRaw.email.subjectPrefix
                      : undefined
              }
            : undefined
        }
      : undefined;
    config.healthCheck = {
      enabled: normalizeBoolean(healthCheck.enabled),
      intervalSeconds:
      ...
      parallelism:
        typeof parallelismRaw === 'number'
          ...
          : typeof parallelismRaw === 'string' && parallelismRaw.trim()
            ...
            : undefined,
      unauthorizedAction:
        typeof unauthorizedRaw === 'string'
          ? (unauthorizedRaw.trim() as 'delete' | 'disable' | 'ignore')
          : undefined,
      zeroQuotaAction:
        typeof zeroQuotaRaw === 'string'
          ? (zeroQuotaRaw.trim() as 'disable' | 'ignore')
          : undefined,
      providerPolicies,
      notifications
    };
