...
export const HealthCheckProviderPoliciesEditor = memo(function HealthCheckProviderPoliciesEditor({
  value,
  disabled,
  onChange,
}: {
  ...
  const unauthorizedOptions = [
    { value: 'default', label: t('config_management.visual.sections.health.use_default') },
    { value: 'disable', label: t('health_check.disabled') },
    { value: 'delete', label: t('config_management.visual.common.delete') },
    { value: 'ignore', label: t('config_management.visual.sections.health.ignore') },
  ] as const;
  const zeroQuotaOptions = [
    { value: 'default', label: t('config_management.visual.sections.health.use_default') },
    { value: 'disable', label: t('health_check.disabled') },
    { value: 'ignore', label: t('config_management.visual.sections.health.ignore') },
  ] as const;
  ...
  const addItem = () =>
    onChange([
      ...value,
      {
        id: makeClientId(),
        provider: '',
        unauthorizedAction: 'default',
        zeroQuotaAction: 'default',
      },
    ]);
  ...
  return (
    <div className={styles.blockStack}>
        ...
        <div key={item.id} className={styles.ruleCard}>
          <div className={styles.providerPolicyRow}>
            <input
              ...
              aria-label={t('config_management.visual.sections.health.provider')}
              value={item.provider}
              onChange={(e) => updateItem(index, { provider: e.target.value })}
              disabled={disabled}
            />
            <Select
              value={item.unauthorizedAction}
              options={[...unauthorizedOptions]}
              disabled={disabled}
              ariaLabel={t('health_check.unauthorized_action')}
              onChange={(nextValue) =>
                updateItem(index, {
                  unauthorizedAction: nextValue as 'default' | 'delete' | 'disable' | 'ignore',
                })
              }
            />
            <Select
              value={item.zeroQuotaAction}
              options={[...zeroQuotaOptions]}
              disabled={disabled}
              ariaLabel={t('health_check.zero_quota_action')}
              onChange={(nextValue) =>
                updateItem(index, {
                  zeroQuotaAction: nextValue as 'default' | 'disable' | 'ignore',
                })
              }
            />
            <Button
      ...
      <div className={styles.actionRow}>
        <Button variant="secondary" size="sm" onClick={addItem} disabled={disabled}>
          {t('config_management.visual.sections.health.add_provider_policy')}
        </Button>
      </div>
    </div>
  );
