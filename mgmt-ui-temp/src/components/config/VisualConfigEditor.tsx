...
import {
  ApiKeysCardEditor,
  HealthCheckProviderPoliciesEditor,
...
export function VisualConfigEditor({
  ...
  const healthCheckIntervalError = getValidationMessage(
    t,
    validationErrors?.['healthCheck.intervalSeconds']
  );
  const healthCheckParallelismError = getValidationMessage(
    t,
    validationErrors?.['healthCheck.parallelism']
  );
  const healthCheckEmailSmtpPortError = getValidationMessage(
    t,
    validationErrors?.['healthCheck.email.smtpPort']
  );
  ...
  const sections = useMemo<VisualSection[]>(
    () => [
      ...
      {
        id: 'health',
        title: t('config_management.visual.sections.health.title'),
        description: t('config_management.visual.sections.health.description'),
        icon: IconTimer,
      ...
      <div ref={workspaceRef} className={styles.workspace}>
        ...
        <div className={styles.sections}>
          ...
          <ConfigSection
            id="health"
            ref={(node) => {
              sectionRefs.current.health = node;
            }}
            indexLabel="08"
            icon={<IconTimer size={16} />}
            title={t('config_management.visual.sections.health.title')}
          ...
          >
            <SectionStack>
              <SectionSubsection
                title={t('config_management.visual.sections.health.basic_title')}
                description={t('config_management.visual.sections.health.basic_desc')}
              >
                <SectionGrid>
                  <ToggleRow
                    title={t('config_management.visual.sections.health.enabled')}
                    description={t('config_management.visual.sections.health.enabled_desc')}
                    checked={values.healthCheck.enabled}
                    disabled={disabled}
                    onChange={(enabled) =>
                      onChange({ healthCheck: { ...values.healthCheck, enabled } })
                    }
                  />
                  <Input
                    label={t('config_management.visual.sections.health.interval')}
                    type="number"
                    placeholder="1800"
                    value={values.healthCheck.intervalSeconds}
                    onChange={(e) =>
                      onChange({
                        healthCheck: {
                          ...values.healthCheck,
                    ...
                    disabled={disabled}
                    error={healthCheckIntervalError}
                  />
                  <Input
                    label={t('config_management.visual.sections.health.parallelism')}
                    type="number"
                    placeholder="4"
                    value={values.healthCheck.parallelism}
                    onChange={(e) =>
                      onChange({
                        healthCheck: {
                          ...values.healthCheck,
                          parallelism: e.target.value,
                        },
                      })
                    }
                    disabled={disabled}
                  ...
                  <FieldShell label={t('health_check.unauthorized_action')}>
                    <Select
                      ...
                      options={[
                        { value: 'disable', label: t('health_check.disabled') },
                        { value: 'delete', label: t('config_management.visual.common.delete') },
                        {
                          value: 'ignore',
                          label: t('config_management.visual.sections.health.ignore'),
                        },
                      ]}
                      disabled={disabled}
                      onChange={(unauthorizedAction) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                            unauthorizedAction: unauthorizedAction as
                              | 'delete'
                              | 'disable'
                              | 'ignore',
                          },
                  ...
                  <FieldShell label={t('health_check.zero_quota_action')}>
                    <Select
                      ...
                      disabled={disabled}
                      onChange={(zeroQuotaAction) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                            zeroQuotaAction: zeroQuotaAction as 'disable' | 'ignore',
                          },
                        })
                      }
                    />
              ...
              <SectionSubsection
                title={t('config_management.visual.sections.health.provider_policies')}
                description={t('config_management.visual.sections.health.provider_policies_desc')}
              >
                <HealthCheckProviderPoliciesEditor
                  value={values.healthCheck.providerPolicies}
                  disabled={disabled}
                  onChange={(providerPolicies) =>
                    onChange({ healthCheck: { ...values.healthCheck, providerPolicies } })
                  }
                />
              </SectionSubsection>

              <SectionSubsection
                title={t('config_management.visual.sections.health.notifications')}
                description={t('config_management.visual.sections.health.notifications_desc')}
              >
                <SectionStack>
                  <ToggleRow
                    title={t('health_check.bark')}
                    checked={values.healthCheck.notifications.barkEnabled}
                    disabled={disabled}
                    onChange={(barkEnabled) =>
                      onChange({
                        healthCheck: {
                          ...values.healthCheck,
                          notifications: {
                  ...
                  <SectionGrid>
                    ...
                    />
                    <Input
                      label={t('health_check.bark_group')}
                      value={values.healthCheck.notifications.barkGroup}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                  ...
                  <ToggleRow
                    ...
                    checked={values.healthCheck.notifications.emailEnabled}
                    disabled={disabled}
                  ...
                  <SectionGrid>
                    <Input
                      label={t('health_check.smtp_host')}
                      value={values.healthCheck.notifications.emailSmtpHost}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                    ...
                    />
                    <Input
                      label={t('health_check.smtp_port')}
                      type="number"
                      value={values.healthCheck.notifications.emailSmtpPort}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                      ...
                      disabled={disabled}
                      error={healthCheckEmailSmtpPortError}
                    />
                    <Input
                      label={t('health_check.smtp_username')}
                      value={values.healthCheck.notifications.emailUsername}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                    ...
                    />
                    <Input
                      label={t('health_check.smtp_password')}
                      type="password"
                      value={values.healthCheck.notifications.emailPassword}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                    ...
                    />
                    <Input
                      label={t('health_check.email_from')}
                      value={values.healthCheck.notifications.emailFrom}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                    ...
                    />
                    <Input
                      label={t('health_check.subject_prefix')}
                      value={values.healthCheck.notifications.emailSubjectPrefix}
                      onChange={(e) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
                  ...
                  <FieldShell label={t('health_check.email_to')}>
                    <StringListEditor
                      value={values.healthCheck.notifications.emailTo}
                      disabled={disabled}
                      placeholder="user@example.com"
                      inputAriaLabel={t('health_check.email_to')}
                      onChange={(emailTo) =>
                        onChange({
                          healthCheck: {
                            ...values.healthCheck,
