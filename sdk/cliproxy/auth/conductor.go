...
// Delete removes an auth entry from memory, scheduler state, and persistent storage.
func (m *Manager) Delete(ctx context.Context, id string) error {
	...
	if m.store != nil {
		target := strings.TrimSpace(auth.FileName)
		if target == "" {
			target = id
		}
		if err := m.store.Delete(ctx, target); err != nil {
			return err
		}
	}
	return nil
