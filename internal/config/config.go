...
const (
	DefaultPanelGitHubRepository = "https://github.com/Dorixxx/clima.git"
...
type Config struct {
	...
	// HealthCheck controls periodic credential health scans exposed in management APIs.
...
// HealthCheckConfig configures periodic auth health validation.
type HealthCheckConfig struct {
	...
	ZeroQuotaAction    string                               `yaml:"zero-quota-action" json:"zero-quota-action"`
	ProviderPolicies   map[string]HealthCheckProviderPolicy `yaml:"provider-policies,omitempty" json:"provider-policies,omitempty"`
	Notifications      HealthCheckNotificationsConfig       `yaml:"notifications,omitempty" json:"notifications,omitempty"`
}

// HealthCheckProviderPolicy overrides health-check actions for one provider/auth type.
type HealthCheckProviderPolicy struct {
	UnauthorizedAction string `yaml:"unauthorized-action,omitempty" json:"unauthorized-action,omitempty"`
	ZeroQuotaAction    string `yaml:"zero-quota-action,omitempty" json:"zero-quota-action,omitempty"`
}

// HealthCheckNotificationsConfig controls completion notifications after each health-check run.
type HealthCheckNotificationsConfig struct {
	Bark  HealthCheckBarkNotificationConfig  `yaml:"bark,omitempty" json:"bark,omitempty"`
	Email HealthCheckEmailNotificationConfig `yaml:"email,omitempty" json:"email,omitempty"`
}
...
// HealthCheckEmailNotificationConfig configures SMTP email notifications.
type HealthCheckEmailNotificationConfig struct {
	Enabled       bool     `yaml:"enabled" json:"enabled"`
	SMTPHost      string   `yaml:"smtp-host,omitempty" json:"smtp-host,omitempty"`
	SMTPPort      int      `yaml:"smtp-port,omitempty" json:"smtp-port,omitempty"`
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	cfg.HealthCheck.IntervalSeconds = 1800
	cfg.HealthCheck.Parallelism = 4
	cfg.HealthCheck.UnauthorizedAction = "disable"
	cfg.HealthCheck.ZeroQuotaAction = "disable"
...
// SanitizeHealthCheck normalizes health-check defaults and enum-like options.
func (cfg *Config) SanitizeHealthCheck() {
	if cfg == nil {
		return
	}
	...
	if cfg.HealthCheck.Parallelism <= 0 {
		cfg.HealthCheck.Parallelism = 4
	}
	if cfg.HealthCheck.Parallelism > 64 {
		cfg.HealthCheck.Parallelism = 64
	}
	switch strings.ToLower(strings.TrimSpace(cfg.HealthCheck.UnauthorizedAction)) {
	case "", "disable":
		cfg.HealthCheck.UnauthorizedAction = "disable"
	case "delete", "ignore":
		cfg.HealthCheck.UnauthorizedAction = strings.ToLower(strings.TrimSpace(cfg.HealthCheck.UnauthorizedAction))
	default:
		cfg.HealthCheck.UnauthorizedAction = "disable"
	}
	switch strings.ToLower(strings.TrimSpace(cfg.HealthCheck.ZeroQuotaAction)) {
	case "", "disable":
		cfg.HealthCheck.ZeroQuotaAction = "disable"
	case "ignore":
		cfg.HealthCheck.ZeroQuotaAction = "ignore"
	default:
		cfg.HealthCheck.ZeroQuotaAction = "disable"
	}
	if len(cfg.HealthCheck.ProviderPolicies) == 0 {
		cfg.HealthCheck.ProviderPolicies = nil
	} else {
		sanitized := make(map[string]HealthCheckProviderPolicy, len(cfg.HealthCheck.ProviderPolicies))
		for provider, policy := range cfg.HealthCheck.ProviderPolicies {
			...
			case "delete", "ignore":
				policy.UnauthorizedAction = strings.ToLower(strings.TrimSpace(policy.UnauthorizedAction))
			default:
				policy.UnauthorizedAction = ""
			}
			switch strings.ToLower(strings.TrimSpace(policy.ZeroQuotaAction)) {
			case "", "disable":
				policy.ZeroQuotaAction = ""
			case "ignore":
				policy.ZeroQuotaAction = "ignore"
			default:
				policy.ZeroQuotaAction = ""
			}
			sanitized[key] = policy
		}
		if len(sanitized) == 0 {
			cfg.HealthCheck.ProviderPolicies = nil
		} else {
	...
	cfg.HealthCheck.Notifications.Bark.Group = strings.TrimSpace(cfg.HealthCheck.Notifications.Bark.Group)
	cfg.HealthCheck.Notifications.Email.SMTPHost = strings.TrimSpace(cfg.HealthCheck.Notifications.Email.SMTPHost)
	if cfg.HealthCheck.Notifications.Email.SMTPPort <= 0 {
		cfg.HealthCheck.Notifications.Email.SMTPPort = 587
	}
	cfg.HealthCheck.Notifications.Email.Username = strings.TrimSpace(cfg.HealthCheck.Notifications.Email.Username)
	cfg.HealthCheck.Notifications.Email.Password = strings.TrimSpace(cfg.HealthCheck.Notifications.Email.Password)
	cfg.HealthCheck.Notifications.Email.From = strings.TrimSpace(cfg.HealthCheck.Notifications.Email.From)
	cfg.HealthCheck.Notifications.Email.SubjectPrefix = strings.TrimSpace(cfg.HealthCheck.Notifications.Email.SubjectPrefix)
	if cfg.HealthCheck.Notifications.Email.SubjectPrefix == "" {
		cfg.HealthCheck.Notifications.Email.SubjectPrefix = "[CLIProxyAPI]"
	}
	if len(cfg.HealthCheck.Notifications.Email.To) > 0 {
		recipients := make([]string, 0, len(cfg.HealthCheck.Notifications.Email.To))
		seen := make(map[string]struct{}, len(cfg.HealthCheck.Notifications.Email.To))
		for _, recipient := range cfg.HealthCheck.Notifications.Email.To {
		...
		cfg.HealthCheck.Notifications.Email.To = recipients
	}
}
